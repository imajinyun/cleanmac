...
pytest-parity-test:
	...
	CLEANMAC_TEST_MODE=1 CLEANMAC_TEST_NO_AUTH=1 PYTHONDONTWRITEBYTECODE=1 PYTEST_ADDOPTS="-p no:cacheprovider" "$$tmpdir/venv/bin/python" -m pytest test_cleanmac.py tests -q
...
no-cache-check:
	...
	"$$venv_python" -m pytest test_cleanmac.py tests -q -p no:cacheprovider; \
