.PHONY: format lint type-check coverage quality-check local-test pytest-parity-test pytest-test pytest-governance-smoke build-check package-smoke script-smoke bundle-audit-smoke macos-smoke real-macos-smoke security-smoke dependency-audit-smoke docs-smoke governance-smoke ai-governance-smoke ai-contract-smoke governed-execution-smoke open-source-smoke mcp-smoke ai-host-smoke ai-robustness-smoke distribution-smoke homebrew-formula-smoke release-artifacts-smoke release-readiness-contract-smoke release-readiness-smoke release-diagnostics-smoke release-rehearsal-smoke release-promotion-smoke release-rollback-smoke release-post-publish-smoke no-cache-check docker-test no-cache-docker-test release-check no-cache-release-check
...
pytest-governance-smoke:
	$(PYTHON) -c 'from pathlib import Path; text=Path("Makefile").read_text(encoding="utf-8"); expected=["tests/test_release_readiness.py", "tests/test_release_orchestration.py", "tests/test_release_artifacts.py"]; line=next(row for row in text.splitlines() if row.startswith("PYTEST_SAFE_TARGETS :=")); targets=line.split(":=", 1)[1].split(); old_all="pytest test_cleanmac.py " + "tests -q"; old_broad="pytest " + "tests -q"; assert targets == expected, targets; [(_ for _ in ()).throw(AssertionError(f"missing pytest safe target: {target}")) for target in targets if not Path(target).is_file()]; assert "pytest $$(PYTEST_SAFE_TARGETS) -q" in text; assert old_all not in text; assert old_broad not in text; assert "pytest-governance-smoke" in next(row for row in text.splitlines() if row.startswith("release-check:")); print("pytest-governance-smoke passed")'
...
release-rollback-smoke:
	CLEANMAC_TEST_MODE=1 CLEANMAC_TEST_NO_AUTH=1 $(PYTHON) -c 'import json, subprocess, sys; plan=json.loads(subprocess.check_output([sys.executable, "cleanmac.py", "--json", "release-rollback-plan"], text=True)); assert plan["schema"] == "cleanmac.release-rollback-plan.v1"; assert plan["manual_only"] is True; assert {"pypi", "github-release", "homebrew-tap"}.issubset({surface["id"] for surface in plan["rollback_surfaces"]}); assert "rm -rf" not in json.dumps(plan); print("release-rollback-smoke passed")'

release-post-publish-smoke:
	CLEANMAC_TEST_MODE=1 CLEANMAC_TEST_NO_AUTH=1 $(PYTHON) -c 'import json, subprocess, sys; plan=json.loads(subprocess.check_output([sys.executable, "cleanmac.py", "--json", "release-post-publish-verification"], text=True)); assert plan["schema"] == "cleanmac.release-post-publish-verification.v1"; assert plan["manual_only"] is True; assert {"pypi", "github-release", "homebrew-tap"}.issubset({surface["id"] for surface in plan["verification_surfaces"]}); assert ["cleanmac", "--json", "release-rollback-plan"] in plan["incident_response_entrypoints"]; assert "rm -rf" not in json.dumps(plan); print("release-post-publish-smoke passed")'
...
no-cache-check:
	...
	"$$venv_python" -m pytest $(PYTEST_SAFE_TARGETS) -q -p no:cacheprovider; \
	PYTHON="$$venv_python" PIP_NO_CACHE_DIR=1 PYTEST_ADDOPTS="-p no:cacheprovider" $(MAKE) local-test build-check package-smoke script-smoke bundle-audit-smoke macos-smoke security-smoke dependency-audit-smoke docs-smoke governance-smoke ai-governance-smoke ai-contract-smoke governed-execution-smoke mcp-smoke ai-host-smoke ai-robustness-smoke open-source-smoke release-readiness-contract-smoke release-readiness-smoke release-diagnostics-smoke release-rehearsal-smoke release-promotion-smoke release-rollback-smoke release-post-publish-smoke
...
no-cache-release-check:
	...
	PIP_NO_CACHE_DIR=1 $(MAKE) distribution-smoke homebrew-formula-smoke release-artifacts-smoke release-readiness-contract-smoke release-readiness-smoke release-diagnostics-smoke release-rehearsal-smoke release-promotion-smoke release-rollback-smoke release-post-publish-smoke no-cache-docker-test
