...
ai-contract-smoke:
	CLEANMAC_TEST_MODE=1 CLEANMAC_TEST_NO_AUTH=1 $(PYTHON) -c 'import json, subprocess, sys; run=lambda *args: json.loads(subprocess.check_output([sys.executable, "cleanmac.py", "--json", *args], text=True)); samples=run("ai-contract-samples"); summary=run("ai-readiness")["contract_validation"]; self_test=run("ai-self-test"); assert samples["schema"] == "cleanmac.ai-contract-samples.v1"; assert samples["sample_count"] == len(samples["samples"]); assert all(sample["valid"] for sample in samples["samples"]), samples; assert summary["ready"], summary; assert summary["contract_schema_coverage"]["missing_stable_ai_schema_fragments"] == []; checks={check["id"]: check for check in self_test["checks"]}; assert checks["contract-validation-smoke"]["passed"], checks["contract-validation-smoke"]; print("ai-contract-smoke passed")'
