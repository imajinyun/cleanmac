...
def main() -> int:
    ...
    if args.evidence:
        ...
        post_publish_result = render_release_post_publish_result(
            dist_dir=resolved_dist_dir,
            assets_dir=assets_dir,
        )
        (assets_dir / "RELEASE-POST-PUBLISH-RESULT.json").write_text(
            json.dumps(post_publish_result, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        post_publish_evidence_template = render_release_post_publish_evidence_template(
            dist_dir=resolved_dist_dir,
            assets_dir=assets_dir,
        )
        (assets_dir / "RELEASE-POST-PUBLISH-EVIDENCE.example.json").write_text(
            json.dumps(post_publish_evidence_template, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
