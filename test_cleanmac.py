...
class CleanMacCLITests(unittest.TestCase):
    ...
    def test_ai_schema_registry_covers_public_ai_schemas(self) -> None:
        ...
        self.assertIn("cleanmac.runtime-lifecycle-policy.v1", names)
    ...
    def test_capabilities_describes_commands_and_safety_model(self) -> None:
        ...
        self.assertIn("ai-host-policy", report["commands"])
        ...
        self.assertEqual(report["ai_host_policy"]["schema"], "cleanmac.ai-host-policy.v1")
        self.assertTrue(report["ai_host_policy"]["valid"])
        self.assertIn("cleanmac_execute_plan", report["ai_host_policy"]["auto_call"]["deny"])
        ...
        self.assertEqual(runtime_lifecycle["schema"], "cleanmac.runtime-lifecycle-policy.v1")
        ...
        self.assertIn("resident GUI/TUI workflow", ai_contract["forbidden"])
        self.assertIn("background daemon", ai_contract["forbidden"])
        self.assertIn("unsolicited scheduled scan", ai_contract["forbidden"])
        ...
        self.assertIn("cleanmac_ai_host_policy", tool_names)
    ...
    def test_clean_reports_ai_confirmation_summary_for_dry_run_and_execute(self) -> None:
        ...
        with tmp:
            ...
            self.assertIn("Dry-run found", human_summary["headline"])
            self.assertFalse(human_summary["safe_to_execute"])
            self.assertIn("--execute", human_summary["next_command"])
            self.assertIn("--confirmation-token", human_summary["next_command"])
            self.assertIn(summary["confirmation_token"], human_summary["next_command"])
            self.assertTrue(human_summary["top_reasons_to_review"])
            ...
            self.assertEqual(execute_report["human_summary"]["next_command"], [])
            self.assertFalse(execute_report["human_summary"]["safe_to_execute"])
    ...
    def test_review_generates_selection_file_from_plan(self) -> None:
        ...
        with tmp:
            ...
            self.assertEqual(human_summary["schema"], "cleanmac.human-summary.v1")
            self.assertIn("Review selected 1 of 1", human_summary["headline"])
            self.assertFalse(human_summary["safe_to_execute"])
            self.assertIn("--review-selection-file", human_summary["next_command"])
            self.assertTrue(human_summary["top_reasons_to_review"])
    ...
    def test_ai_workflow_renders_one_shot_governed_safe_cleanup_route(self) -> None:
        tmp, root, home = self.make_sandbox()
        with tmp:
            result = self.run_cli(
                "--root",
    ...
    def test_makefile_exposes_validation_targets(self) -> None:
        ...
        self.assertIn('payload["resource_count"] == 36', makefile)
        ...
        self.assertIn("release-post-publish-result-smoke:", makefile)
        ...
        self.assertIn("make governance-integrity-smoke", makefile)
    ...
    def test_release_workflow_generates_checksums_attestation_and_pypi_publish(self) -> None:
        ...
        self.assertIn("release-assets/RELEASE-POST-PUBLISH-RESULT.json", release)
        ...
        self.assertIn('assert post_publish_result["manual_only"] is True', release)
    ...
    def test_open_source_governance_files_are_configured(self) -> None:
        ...
        self.assertIn("AI-first macOS cleanup CLI", readme)
        self.assertIn("AI discovery summary", readme)
        self.assertIn("AI-first, zero-resident macOS cleanup CLI", readme)
        self.assertIn("MCP-ready execution kernel", readme)
        self.assertIn("What AI agents should know", readme)
        self.assertIn("Recommended GitHub topics for discoverability", readme)
        self.assertIn("model-context-protocol", readme)
        self.assertIn("Common questions cleanmac should answer", readme)
        self.assertIn("Zero-resident by design", readme)
        self.assertIn("no resident GUI, TUI, menu bar process", readme)
        self.assertIn("AI-first macOS 清理 CLI", readme_cn)
        self.assertIn("零常驻的 macOS 清理 CLI", readme_cn)
        self.assertIn("AI Agent 应该如何理解 cleanmac", readme_cn)
        self.assertIn("model-context-protocol", readme_cn)
        self.assertIn("设计上零常驻", readme_cn)
        self.assertIn("不提供常驻 GUI、TUI、菜单栏进程", readme_cn)
        ...
        self.assertIn("## 🤖 Agent Summary for GEO and AI Search", agents)
        self.assertIn("AI-first, zero-resident macOS cleanup CLI", agents)
        self.assertIn("Do not describe cleanmac as a GUI cleaner", agents)
        ...
        self.assertIn("AI-first zero-resident macOS cleanup CLI", pyproject)
        self.assertIn('"ai-first"', pyproject)
        self.assertIn('"model-context-protocol"', pyproject)
        self.assertIn('"zero-resident"', pyproject)
