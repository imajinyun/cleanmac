...
def test_safe_sudo_remove_blocks_no_auth_and_symlinks() -> None:
    ...
    with tmp:
        ...
        with patch.dict("os.environ", {"CLEANMAC_TEST_NO_AUTH": "1"}, clear=False):
            with pytest.raises(RuntimeError, match="sudo blocked"):
...
def test_require_trash_first_delete_mode_blocks_permanent_without_explicit_gate() -> None:
    ...
    with pytest.raises(RuntimeError, match="requires --delete-mode trash"):
