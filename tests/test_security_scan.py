...
def test_security_scan_flags_raw_delete_and_privileged_business_calls(tmp_path: Path) -> None:
    ...
    assert any("subprocess must not directly invoke rm" in violation for violation in violations), violations
...
def test_security_scan_flags_python_shell_strings_and_absolute_commands(tmp_path: Path) -> None:
    ...
    assert any("privileged command 'osascript'" in violation for violation in violations), violations
    assert any("subprocess must not directly invoke rm" in violation for violation in violations), violations
...
def test_security_scan_flags_workflow_privileged_run_blocks(tmp_path: Path) -> None:
    ...
    assert any("workflow must not invoke privileged command 'launchctl'" in violation for violation in violations), (
        violations
    )
    assert any("workflow must not invoke privileged command 'osascript'" in violation for violation in violations), (
        violations
    )
...
def test_security_scan_flags_gui_tui_and_resident_product_surfaces(tmp_path: Path) -> None:
    ...
    assert any("forbidden GUI framework import 'tkinter'" in violation for violation in violations), violations
    assert any("forbidden menu bar app framework import 'rumps'" in violation for violation in violations), violations
    assert any("forbidden TUI framework dependency 'textual'" in violation for violation in violations), violations
