...
Xcode/iOS candidate evidence returns schema `cleanmac.xcode-ios-candidates.v1`. It is read-only and emits bounded summary fields such as `candidate_count_by_path_role`, `candidate_count_by_default_policy`, `estimated_bytes_by_path_role`, `shown_candidate_count`, `truncated`, `scan_duration_ms`, and `next_review_command`. Candidate details carry the same governance fields declared by `cleanmac.xcode-ios-governance.v1`, including `path_role`, `tool_domain`, `regenerable`, `contains_user_data`, `release_artifact_risk`, `active_runtime_hint`, `retention_reason`, `default_selected`, `why_not_default`, and `recommended_next_action`.

The command does not create executable cleanup plans. Xcode Archives, DeviceSupport, iOS backups, and unavailable simulator devices remain never default selected and must be reviewed through `cleanmac --json review --input-file <xcode-ios-candidates.json>` before any future execution design.
...
make xcode-ios-governance-smoke                    # Read-only Xcode/iOS governance gate
make xcode-ios-candidates-smoke                    # Read-only Xcode/iOS candidate evidence
make zero-resident-audit-smoke                     # Zero-resident product boundary audit
make no-disturbance-smoke                           # No-disturbance silent-by-default gate
...
| `xcode-ios-governance-smoke` | Read-only Xcode/iOS governance contract |
| `xcode-ios-candidates-smoke` | Read-only Xcode/iOS candidate evidence contract |
...
| `no-disturbance-smoke` | No-disturbance silent-by-default contract and release gate |
