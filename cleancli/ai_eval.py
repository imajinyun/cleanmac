    ...
    scenarios = [
        {
            ...
            "expected_final_schema": "cleanmac.ai-tool-decision-matrix.v1",
...
def selected_scenario_ids(requested: str, all_ids: Sequence[str]) -> list[str]:
    if requested == "smoke":
        return ["discover_readiness", "safe_plan_to_dry_run", "invalid_category_recovery", "confirmation_token_policy"]
    if requested == "all":
        return list(all_ids)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if "discover_readiness" in selected:
            ...
            results.append(
                {
                    "id": "discover_readiness",
                    "passed": bool(readiness["ready"] and matrix["violation_count"] == 0),
                    "observed_schema": matrix["schema"],
        ...
        if "safe_plan_to_dry_run" in selected or "confirmation_token_policy" in selected:
            plan, event = _run_cli(
                cli,
