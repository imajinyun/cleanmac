        ...
        "parameters": object_schema(
            {
                ...
                "review_selection_file": string_schema(
                    "Optional cleanmac.review-selection.v1 file to constrain execution to reviewed selected items."
                ),
                "require_plan_context": {
                    "type": "boolean",
                    "description": "Require root/home context match before execution.",
...
def build_tool_argv(name: str, args: Mapping[str, Any] | None = None) -> list[str]:
    ...
    if name == "cleanmac_review":
        ...
        append_option(argv, args, "item_scope", "--item-scope")
        append_option(argv, args, "item_sort", "--item-sort")
        return argv
    if name == "cleanmac_scripts":
